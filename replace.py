...
for img in glob.glob('*.jpg'):
	...
	ww, hh = img_gray.shape[::-1]

	template_bgr = cv2.imread('_/t.jpg')
	template_gray = cv2.imread('_/t.jpg',0)
	template_canny = cv2.Canny(template_gray, 30, 200)
	w, h = template_gray.shape[::-1]

	# Match
	res = cv2.matchTemplate(img_canny, template_canny, cv2.TM_CCORR_NORMED)
	...
	#
	box = img_bgr[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]
	...
	# cv2.imshow("template", i1)
	# cv2.imshow("cropped", i2)

	# Normalized cross-correlation
	dist_ncc = np.sum( (i1 - np.mean(i1)) * (i2 - np.mean(i2)) ) / ((i1.size - 1) * np.std(i1) * np.std(i2) )


	percents = [max_val, dist_ncc]

	#
	print("matchTemplate:" , f'{max_val:.4f}', f'{dist_ncc:.4f}', f'{sum(percents)/len(percents):.2f}', img)

	if (sum(percents)/len(percents) > 0.3 ):
		img_mask = img_gray.copy()
		cv2.rectangle(img_mask,(0,0), (ww,hh), (255,255,255), cv2.FILLED )
		# cv2.rectangle(img_mask,top_left, bottom_right, (255,255,255), cv2.FILLED )
		# img_mask((0,0), (40,35)) = template_gray

		img_mask[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]] = template_gray
		...
		# cv2.floodFill(img_mask, mask, (0,0), 255);
		...
		# img_bgr = img_mask
		img_bgr = cv2.inpaint(img_bgr,img_mask,3,cv2.INPAINT_NS)
		...
		#
		cv2.imshow('img_bgr-'+ img, img_bgr)
